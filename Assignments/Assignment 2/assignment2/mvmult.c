...
int main(void) {
    ...
    MPI_Barrier(comm);    // Stop the program until all processes reach this point
    start = MPI_Wtime();  // Start timing

    Mat_vect_mult(local_A, local_x, local_y, local_m, n, local_n, comm);

    finish = MPI_Wtime();          // Finish timing
    loc_elapsed = finish - start;  // Calculate elapsed time of the matrix-vector multiplication in each process

#ifdef DEBUG
    Print_vector("y", local_y, m, local_m, my_rank, comm);
#endif

    /*  ALW your timing code will compute "elapsed"
     */
    MPI_Reduce(&loc_elapsed, &elapsed, 1, MPI_DOUBLE, MPI_MAX, 0, comm);  // Find the maximum elapse time of all processes
